    ...
    network.request({
      ...
      success(res) {
        if (res.data.code == 100010) {
          ...
          const foodArr = changeCarData(arr);
          that.setData({
            foods: foodArr,
            ids,
            totalNum: arr.length,
    ...
    network.request({
      ...
      success(res) {
        if (res.data.code == 100020) {
          wx.redirectTo({
            url: `/pages/hotel/success/index?id=${res.data.data.id}&order=${res.data.data.order_id}&time=${res.data.data.eat_time}&price=${res.data.data.price}`
          });
        } else {
