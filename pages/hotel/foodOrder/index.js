...
Page({
  ...
  goDetails(e) {
    wx.navigateTo({
      url: `/pages/hotel/foodOrderDetails/index?id=${e.currentTarget.id}`
    });
  },

  goEval(e) {
    wx.navigateTo({
      url: `/pages/hotel/foodEval/index?id=${e.currentTarget.id}`
    });
  },
  ...
  onShow: function () {
    ...
    if (this.data._initialized) {
      ...
      this.getData({}, 'all');
      this.getData({ status: 0 }, 'pay');
      this.getData({ status: 1 }, 'check');
      this.getData({ status: 2 }, 'finish');
