...
exports.buildTaskMonitorPage = buildTaskMonitorPage;
const layout_1 = require("../webview/layout");
function buildTaskMonitorPage(data) {
...
${schedulerJobs.length ? schedulerJobs.map((job) => `
...
${allTasks.map((task) => `<button class="btn" onclick="executeTask('${task.name}')" title="${task.description || ''}">${task.name}</button>`).join('')}
</div></div></div>
...
${taskStats.length > 0 ? taskStats.map((t) => `
...
document.addEventListener('visibilitychange', () => {
    ...
    return (0, layout_1.pageShell)('taskmonitor', 'Task Monitor · 任务监控', content, extraScript);
}
