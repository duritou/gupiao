...
function buildBacktestPage(data) {
    ...
    const winCount = trades.filter((t) => t.profit_pct > 0).length;
...
${trades.slice(0, 15).map((t) => `<span style="font-size:20px;color:${t.profit_pct > 0 ? '#3fb950' : '#f85149'}" title="${t.entry_date || ''} → ${t.exit_date || ''}: ${t.profit_pct > 0 ? '+' : ''}${(t.profit_pct || 0).toFixed(1)}% · ${t.exit_reason || ''}">${t.profit_pct > 0 ? '✔' : '✘'}</span>`).join('')}
</div>
...
<td>${t.holding_days || '-'}天</td><td>${t.exit_reason || '-'}</td>
    ...
    return (0, layout_1.pageShell)('backtest', 'Backtest · 策略验证', content);
}
