...
exports.buildNewsRadarPage = buildNewsRadarPage;
const layout_1 = require("../webview/layout");
function buildNewsRadarPage(data) {
...
<span>${news.time || ''}</span>
${news.stocks && news.stocks.length > 0 ? `<span style="margin-left:12px">${news.stocks.map((s) => `<span class="tag" style="background:#1a1a1a;padding:2px 8px;border-radius:4px;font-size:11px">${s}</span>`).join(' ')}</span>` : ''}
</div>
</div>
...
function openNewsLink(url) {
    ...
    return (0, layout_1.pageShell)('newsradar', 'News Radar · 新闻雷达', content, extraScript);
}
//# sourceMappingURL=newsradar.js.map