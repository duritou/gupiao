...
${hotSectors.map((s) => `<span class="tag tag-up" style="font-size:14px;padding:6px 14px">${'★'.repeat(s.stars || 1)} ${s.name} ${s.score}</span>`).join('') || '<span class="text-muted">加载中...</span>'}
</div></div>
...
async function refreshBrief() {
    try {
        await fetch('${constants_1.BASE_URL}/dailybrief/generate', {method:'POST'});
    } catch(e) {}
    location.reload();
}`;
