...
const chart_timeline_1 = require("../chart/chart-timeline");
...
function buildResearchPage(code, detail) {
    ...
    const aiScore = (0, score_display_1.finiteScore)(d.ai_score);
    const sc = (0, score_display_1.scoreTone)(aiScore);
    ...
    const klineData = (d.klines && d.klines.length > 0)
        ? d.klines.map((k) => ({ date: k.date, open: k.open, high: k.high, low: k.low, close: k.close, volume: k.volume }))
        : [];
...
<div class="score-num ${sc}">${(0, score_display_1.scoreText)(aiScore)}</div>
...
<div class="indicator-item"><span class="ind-label">RSI</span><span class="ind-value">${indicators.rsi?.value ?? ((0, score_display_1.finiteScore)(scores.rsi) === null ? 'N/A' : (0, score_display_1.finiteScore)(scores.rsi).toFixed(0))} <span style="font-size:10px;color:#9CA3AF">${indicators.rsi?.status || ((0, score_display_1.finiteScore)(scores.rsi) === null ? '数据不可用' : '健康')}</span></span></div>
