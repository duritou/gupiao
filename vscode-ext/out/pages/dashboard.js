...
function buildDashboardPage(data) {
    ...
    const healthStatus = effectiveAvailable
...
${urgentAlerts.slice(0, 2).map((a) => _renderFocusAlert(a, true)).join('')}
${importantAlerts.slice(0, Math.max(0, 3 - urgentAlerts.length)).map((a) => _renderFocusAlert(a, false)).join('')}
</div>` : `
