...
exports.buildConnectorsPage = buildConnectorsPage;
const layout_1 = require("../webview/layout");
const constants_1 = require("../constants");
function buildConnectorsPage(data) {
...
${ranking.length > 0 ? ranking.map((p) => `
...
async function refreshConnectors() {
    try {
        await fetch('${constants_1.BASE_URL}/market/data-status');
        await fetch('${constants_1.BASE_URL}/market/registry');
...
startAutoRefresh();`;
    return (0, layout_1.pageShell)('connectors', 'Data Connectors · 数据连接器', content, extraScript);
}
