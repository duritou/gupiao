...
<div id="alertsFeed">
${alerts.map((a) => _renderAlertCard(a)).join('') || '<div class="empty-state"><div class="icon">🔔</div><p>暂无预警 · AI持续监控中</p></div>'}
</div>
...
async function refreshAlerts() {
    try {
        let url = '${constants_1.BASE_URL}/alerts?limit=50';
...
async function markRead(alertId) {
    try {
        await fetch('${constants_1.BASE_URL}/alerts/' + alertId + '/read', { method: 'POST' });
        refreshAlerts();
    } catch(e) {}
}
...
async function dismissAlert(alertId) {
    try {
        await fetch('${constants_1.BASE_URL}/alerts/' + alertId + '/dismiss', { method: 'POST' });
        refreshAlerts();
    } catch(e) {}
}
...
async function recordAction(alertId, actionType, stockCode) {
    try {
        await fetch('${constants_1.BASE_URL}/alerts/' + alertId + '/action', {
            method: 'POST',
...
function renderAlertCard(a) {
    ...
    return (0, layout_1.pageShell)('alerts', 'Alert Intelligence · 智能预警中心', content, extraScript);
}
...
${a.tags ? a.tags.slice(0, 3).map((t) => `<span style="font-size:10px;padding:1px 6px;border-radius:8px;background:#21262d;color:#8b949e">${t}</span>`).join('') : ''}
</div>
...
function _levelCategoryLabel(a) {
    const labels = {
