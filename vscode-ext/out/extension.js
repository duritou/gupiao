...
function activate(context) {
    ...
    statusBar.show();
    context.subscriptions.push(vscode.commands.registerCommand('quantai.terminal', () => showTerminal(context, 'dashboard')), vscode.commands.registerCommand('quantai.dashboard', () => showTerminal(context, 'dashboard')), vscode.commands.registerCommand('quantai.watchlist', () => showTerminal(context, 'watchlist')), vscode.commands.registerCommand('quantai.research', () => showStockResearch(context)), vscode.commands.registerCommand('quantai.marketmap', () => showTerminal(context, 'marketmap')), vscode.commands.registerCommand('quantai.alerts', () => showTerminal(context, 'alerts')), vscode.commands.registerCommand('quantai.backtest', () => showTerminal(context, 'backtest')), vscode.commands.registerCommand('quantai.dailybrief', () => showTerminal(context, 'dailybrief')), vscode.commands.registerCommand('quantai.startServer', startServer), vscode.commands.registerCommand('quantai.addWatch', addToWatchlist));
    // Sidebar
...
async function showStockResearch(context) {
...
// WEBVIEW PANEL
// ============================================================
function createOrShowPanel(context, title, html) {
    if (currentPanel) {
...
function buildNav(active) {
    ...
    const labels = { dashboard: 'Dashboard', watchlist: 'Watchlist', marketmap: 'Market Map', alerts: 'Alerts', backtest: 'Backtest', dailybrief: 'Daily Brief' };
    return `<div class="nav">${items.map(i => `<span class="nav-item${i === active ? ' active' : ''}" onclick="navigate('${i}')">${labels[i]}</span>`).join('')}</div>`;
}
// ---- DASHBOARD ----
function buildDashboard(css, nav, data) {
...
${['AI人工智能', '半导体芯片', '电力设备', '医药生物', '机器人'].map((h, i) => `<div class="stock-row"><span>${'&#x2605;'.repeat(5 - i)} ${h}</span><span class="tag tag-up">活跃</span></div>`).join('')}</div>
<div class="card"><h3>风险预警</h3>
${[{ t: '高位放量', c: 4, s: 'warn' }, { t: '跌破MA20', c: 8, s: 'down' }, { t: '机构减仓', c: 5, s: 'warn' }, { t: '北向流出', c: 3, s: 'down' }].map(r => `<div class="stock-row"><span>${r.t}</span><span class="${r.s}">${r.c}只</span></div>`).join('')}</div>
</div>
...
// ---- WATCHLIST ----
function buildWatchlist(css, nav, data) {
...
// Auto-load scores
${stocks.map((code, i) => `fetch('${BASE_URL}/signals/compute/${code}?trend=up').then(r=>r.json()).then(d=>{
...
}).catch(()=>{document.getElementById('score${i}').textContent='N/A'})`).join(';\n')}
</script></body></html>`;
}
// ---- MARKET MAP ----
function buildMarketMap(css, nav, data) {
    const sectors = [
...
// ---- ALERTS ----
function buildAlerts(css, nav, data) {
    const alerts = [
...
// ---- BACKTEST ----
function buildBacktest(css, nav, data) {
...
// ---- DAILY BRIEF ----
function buildDailyBrief(css, nav, data) {
    const s = data.scanner || {};
...
${candidates.map((c, i) => `<div class="stock-row"><span>#${i + 1} ${c.stock_name || c.stock_code}</span><span class="${c.fusion_score >= 70 ? 'up' : 'neutral'}" style="font-weight:700">${c.fusion_score?.toFixed(0)}分</span></div>`).join('')}</div>
...
function buildResearchPage(code, signals, research) {
    const s = signals || {};
...
${Object.entries(s.scores || {}).map(([k, v]) => `<div style="margin:8px 0"><div style="display:flex;justify-content:space-between"><span>${k.toUpperCase()}</span><span class="${v >= 70 ? 'up' : v >= 40 ? 'neutral' : 'down'}">${v?.toFixed(0)}</span></div>
...
${(s.reasons || []).map((r) => `<li style="padding:6px 0;border-bottom:1px solid #21262d">&bull; ${r}</li>`).join('')}</ul></div>
</div>
