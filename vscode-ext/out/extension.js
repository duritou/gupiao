...
function activate(context) {
    ...
    // 注册所有命令
    context.subscriptions.push(vscode.commands.registerCommand('quantai.startServer', startServer), vscode.commands.registerCommand('quantai.stopServer', stopServer), vscode.commands.registerCommand('quantai.scan', () => runScanner(context)), vscode.commands.registerCommand('quantai.analyze', () => analyzeStock(context)), vscode.commands.registerCommand('quantai.research', () => runResearch(context)), vscode.commands.registerCommand('quantai.backtest', () => runBacktest(context)), vscode.commands.registerCommand('quantai.knowledge', () => searchKnowledge(context)), vscode.commands.registerCommand('quantai.status', showStatus));
    // 注册侧边栏
...
// COMMANDS
// ============================================================
async function runScanner(context) {
...
async function analyzeStock(context) {
...
async function searchKnowledge(context) {
...
async function showStatus() {
    try {
        ...
        vscode.window.showInformationMessage(`AI Research Terminal v${data.version} — ${data.modules.length} 模块就绪`, '查看详情').then(sel => {
            if (sel === '查看详情') {
                const panel = vscode.window.createWebviewPanel('quantaiStatus', '系统状态', vscode.ViewColumn.One, { enableScripts: true });
                panel.webview.html = `<html><body style="padding:20px;font-family:monospace">
                    ...
                    ${data.modules.map((m) => `<li>${m}</li>`).join('')}</ul></body></html>`;
            }
        });
    }
    catch {
...
// WEBVIEW
// ============================================================
function showWebview(context, title, content) {
    const panel = vscode.window.createWebviewPanel('quantaiResult', title, vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
...
<p>${(c.tags || []).map((t) => `<span class="tag">${t}</span>`).join(' ')}</p>
<p>${Object.entries(c.score_breakdown || {}).map(([k, v]) => `${k.toUpperCase()}: <b>${v?.toFixed(0)}</b>`).join(' &nbsp;|&nbsp; ')}</p>
</div>`;
    });
    return html;
}
function formatSignalResult(data) {
...
<h2>各维度评分</h2>
<div>${Object.entries(data.scores || {}).map(([k, v]) => `<div style="margin:8px 0"><b>${k.toUpperCase()}</b>:
    <span style="display:inline-block;width:${v}px;height:16px;background:${v >= 70 ? '#4caf50' : v >= 40 ? '#ff9800' : '#f44336'};border-radius:4px;"></span> ${v?.toFixed(0)}</div>`).join('')}</div>
<h2>信号详情</h2><ul>${(data.reasons || []).map((r) => `<li>${r}</li>`).join('')}</ul>`;
}
...
${(data.trades || []).length > 0 ? '<h2>交易明细</h2>' + (data.trades || []).slice(0, 10).map((t) => `<div class="card">${t.entry} → ${t.exit} | 盈亏: <b>${t.profit_pct?.toFixed(1)}%</b> | 持有${t.holding_days}天 | ${t.exit_reason}</div>`).join('') : '<p>无交易</p>'}`;
}
function formatKnowledgeResult(data) {
    return `<h1>知识库搜索: ${data.query}</h1>
${(data.results || []).map((r) => `<div class="card">
...
function cmdItem(label, command) {
