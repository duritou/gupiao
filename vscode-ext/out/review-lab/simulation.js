...
exports.SYNTHETIC_BARS = void 0;
exports.simulateReview = simulateReview;
/** Tiny deterministic fixtures only. This is not an upstream strategy implementation. */
const model_1 = require("./model");
exports.SYNTHETIC_BARS = [
    { symbol: 'SYNTHETIC_A', previous: 10, close: 11 },
    { symbol: 'SYNTHETIC_B', previous: 20, close: 19 },
    { symbol: 'SYNTHETIC_C', previous: 5, close: 5 },
];
function simulateReview(date, activity, bars = exports.SYNTHETIC_BARS) {
    if (!(0, model_1.validDate)(date))
        throw new Error('无效模拟日期');
    ...
    let cancelled = false;
    return model_1.REVIEW_PROJECTS.map((project, index) => {
        let state = 'unknown';
        try {
            state = activity();
        }
        catch { /* Unknown must fail closed. */ }
