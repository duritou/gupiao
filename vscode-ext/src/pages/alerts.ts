...
async function refreshAlerts() {
    try {
        const resp = await fetch('${BASE_URL}/alerts/recent?limit=50' + (currentFilter !== 'all' ? '&type=' + currentFilter : ''));
...
startAutoRefresh();`;
    ...
    return pageShell('alerts', 'Alert Center · 预警中心', content, extraScript);
}
