...
function displayNumber(value: unknown, available: boolean): string {
...
function latestLearningProfile(entries: any[]): any {
    for (const entry of entries) {
        const evidence = entry?.evidence || {};
        const profile = evidence.learning_profile || evidence.profile;
        if (profile && (profile.total_observations != null || profile.decisive_observations != null)) return profile;
    }
    return {};
}

export function buildAIOSPage(data: any): string {
    ...
    const failedTasks = statusAvailable ? taskStats.filter((task: any) => task.last_status === 'failed') : [];
    const learning = latestLearningProfile(learningEntries);
    const tradesToday = todayAvailable
        ? (today?.paper_portfolio?.trades || []).filter((trade: any) => trade.trade_date === today.date).length
        : null;
    ...
    const learningMetric = (value: unknown): string => {
...
${failedTasks.length ? `<div style="margin-top:10px;padding:8px;background:#3a351b;border-radius:6px;font-size:12px;color:#F59E0B">待处理任务：${failedTasks.map((task: any) => `${escapeHtml(task.task_name)}（最近状态失败）`).join('、')}</div>` : ''}
</div>
...
<div class="card" style="margin-top:16px"><div class="card-header"><h3>📡 决策时间线</h3><span class="text-sm text-muted">最近 ${eventsAvailable ? events.length : '—'} 条</span></div><div style="max-height:400px;overflow-y:auto">${eventsAvailable ? (events.map(renderEventRow).join('') || '<div class="empty-state"><p>暂无事件</p></div>') : '<div class="empty-state"><p>决策事件接口暂不可用，历史记录未删除</p></div>'}</div></div>
    ...
    return pageShell('aios', 'AI OS · 系统运行', content, `function refreshAIOS(){vscode.postMessage({command:'refreshPage'});}`);
}

function renderEventRow(event: any): string {
    const sourceIcons: Record<string, string> = { decision_journal: '📈', market: '📈', scanner: '🔍', alert: '🔔', portfolio: '📦', research: '🔬', user: '👤', trust: '🤖', ai_os: '⚙', system: '🖥' };
