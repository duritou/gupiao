...
${learningLog.length > 0 ? learningLog.slice(0, 8).map((l: string) => `
...
${events.map((e: any) => _renderEventRow(e)).join('') || '<div class="empty-state"><p>暂无事件</p></div>'}
</div>
</div></div>
`;

    return pageShell('aios', 'AI OS · 系统运行', content, '');
}

function _renderEventRow(e: any): string {
