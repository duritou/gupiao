...
export function buildResumePage(data: any): string {
...
${monthly.map((m: any) => `<span style="font-size:10px;color:#6B7280">${m.month.slice(5)}</span>`).join('')}
</div>
</div></div>` : ''}
`;
...
async function refreshResume() {
    try {
        await Promise.all([
            fetch('${BASE_URL}/trust/resume'),
            fetch('${BASE_URL}/trust/model-evolution'),
            fetch('${BASE_URL}/trust/monthly'),
            fetch('${BASE_URL}/trust/strategies'),
            fetch('${BASE_URL}/trust/score-ranges'),
            fetch('${BASE_URL}/trust/track-record?days=30'),
        ]);
    } catch(e) {}
}
refreshResume();`;

    return pageShell('resume', 'AI Resume · 信任档案', content, extraScript);
}

function _renderSparkline(monthly: any[]): string {
    if (!monthly.length) return '数据不足';
