...
${hotSectors.map((s: any) => `<span class="tag tag-up" style="font-size:14px;padding:6px 14px">${'★'.repeat(s.stars || 1)} ${s.name} ${s.score}</span>`).join('') || `<span class="text-muted">${hotSectorFallback}</span>`}
</div></div>
