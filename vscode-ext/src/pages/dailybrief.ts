...
export type DailyBriefState = 'ready' | 'degraded' | 'collecting' | 'empty' | 'request' | 'timeout';

export function getDailyBriefState(data: any): DailyBriefState {
    if (data?.pageError) {
        return data.pageErrorKind === 'timeout' ? 'timeout' : 'request';
    }
    const brief = data?.brief;
    if (!brief || typeof brief !== 'object' || Object.keys(brief).length === 0) return 'empty';
    const declared = brief.data_status?.state;
    if (declared === 'collecting') return 'collecting';
    if (declared === 'empty') return 'empty';
    if (brief.data_status?.available === false) return 'empty';
...
export function buildDailyBriefPage(data: any): string {
    ...
    const decisionDate = String(
        brief.decision_date || pipelineAcceptance.decision_date || brief.date || '',
    ).trim();
    const opportunities = (brief.top_opportunities || []).filter((item: any) => {
        if (item.recommendation_tier === 'technical_watchlist') return false;
        if (decisionDate && item.decision_date && item.decision_date !== decisionDate) return false;
        if (pipelineRunId && item.run_id && item.run_id !== pipelineRunId) return false;
        return true;
    });
...
<span class="${scoreTone(finiteScore(o.score))}" style="font-weight:700;font-size:18px">${scoreText(o.score, '分')}</span><br>
...
${patternWatchlist.map((item: any, i: number) => {
    ...
    const dates = (item.pattern_dates || []).map((value: any) => escapeHtml(value)).join(' → ');
    return `
...
${shortTermSentiment.lianban_ladder ? Object.entries(shortTermSentiment.lianban_ladder).map(([boards, count]: [string, any]) =>
  `<div style="background:#1a1a1a;padding:12px 16px;border-radius:6px;border:1px solid #30363d;min-width:80px;text-align:center">
...
${shortTermSentiment.lianban_stocks.slice(0, 20).map((s: any, i: number) => `
...
${topVolumeStocks.map((s: any, i: number) => {
