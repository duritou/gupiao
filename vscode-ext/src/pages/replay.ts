...
async function runReplay() {
    ...
    try {
        const resp = await fetch('${BASE_URL}/replay/report/' + date);
...
async function runCompare() {
    ...
    try {
        const resp = await fetch('${BASE_URL}/replay/compare', {
            method: 'POST',
...
async function runSimulate() {
    ...
    try {
        const resp = await fetch('${BASE_URL}/replay/simulate', {
            method: 'POST',
            headers: {'Content-Type':'application/json'},
    ...
    return pageShell('replay', 'Replay Engine · 时间机器', content, extraScript);
}

function _renderReport(r: any): string {
    const ctx = r.context || {};
...
${sim.scenario_results ? Object.entries(sim.scenario_results).slice(0, 5).map(([name, res]: [string, any]) => `
...
${sim.insights ? sim.insights.map((i: string) => `<div style="font-size:12px;color:#c9d1d9;margin-top:6px">💡 ${i}</div>`).join('') : ''}
</div></div>
`;
}
...
${data.results ? Object.entries(data.results).map(([name, res]: [string, any]) => `
...
${data.insights ? data.insights.map((i: string) => `<div style="font-size:12px;color:#c9d1d9;margin-top:8px">💡 ${i}</div>`).join('') : ''}
</div></div>`;
}
