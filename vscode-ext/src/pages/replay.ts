...
export function buildReplayPage(data: any): string {
    const datesPayload = data.dates || {};
    const replayDates: any[] = datesPayload.dates || [];
    const models: any[] = datesPayload.models || [];
    const historyRuns: any[] = data.history?.runs || [];
...
updateReplayDateInfo();`;
    ...
    return pageShell('replay', 'Replay · 技术评分口径回放', content, extraScript);
}
...
function _fmtPct(value: any, digits = 1): string {
    if (value === null || value === undefined || Number.isNaN(Number(value))) return '--';
    const number = Number(value);
    return `${number >= 0 ? '+' : ''}${number.toFixed(digits)}%`;
}

function _statusBlock(status: string, message: string): string {
...
function _renderDateCoverage(dates: any[]): string {
...
function _renderHistory(runs: any[]): string {
...
function _renderCandidateTable(rows: any[], horizon: number): string {
    if (!rows?.length) return '<div style="font-size:12px;color:#8b949e">没有可重算的候选。</div>';
...
function _renderComparisonPanel(comparison: any): string {
    const metrics = comparison?.metrics || {};
    const entries = Object.entries(metrics) as [string, any][];
...
function _renderSimulationPanel(simulation: any): string {
    const results = simulation?.scenario_results || simulation?.results || {};
    const entries = Object.entries(results) as [string, any][];
    return `<div class="card">
...
${(simulation?.insights || []).map((insight: string) => `<div style="font-size:11px;color:#c9d1d9;margin-top:7px">💡 ${_escapeHtml(insight)}</div>`).join('')}</div>`;
}

function _renderWechatArticles(articles: any[]): string {
...
function _renderReport(report: any): string {
...
function _renderCompare(data: any): string {
...
function _renderSimulation(data: any): string {
