...
${decisions.map((d: any) => _renderDecisionCard(d, recLabels, recColors)).join('')}
</div>`;

    return pageShell('decisions', 'Decision Center · 决策中心', content, '');
}
...
${(d.bull_points || []).slice(0, 3).map((b: any) => `
...
${(d.bear_points || []).slice(0, 3).map((b: any) => `
<div style="font-size:11px;color:#c9d1d9;margin-bottom:3px;line-height:1.4">
