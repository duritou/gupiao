...
export function buildWatchlistPage(data: any): string {
    ...
    const newest = initialScores.find((s: any) => s.data_date || s.computed_at) || {};
    ...
    liveQuotes.forEach((q: any) => { quoteMap[q.stock_code] = q; });
    ...
    const rows = stocks.map((code: string, i: number) => {
        ...
        const score = finiteScore(s.fusion_score);
...
<td id="score_${i}"><span class="${sc}" style="font-weight:700;font-size:16px" title="自选股技术信号评分">${scoreText(score)}</span><div class="text-sm text-muted">技术分</div></td>
