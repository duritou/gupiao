...
export function buildJournalPage(data: any): string {
...
<div id="journalFeed">
${entries.map((e: any) => _renderJournalEntry(e)).join('') || '<div class="empty-state"><div class="icon">📓</div><p>数据积累中，AI正在学习...</p></div>'}
</div>
</div>`;
...
async function refreshJournal() {
    try {
        let url = '${BASE_URL}/trust/journal?limit=30';
...
document.querySelectorAll('.filter-btn').forEach(b => {
    ...
    return pageShell('journal', 'Decision Journal · 决策日志', content, extraScript);
}

function _renderJournalEntry(e: any): string {
    const s = e.snapshot || {};
...
${s.signals.slice(0, 4).map((sig: any) =>
    `<span style="font-size:10px;padding:2px 8px;border-radius:8px;background:#21262d;color:#8b949e">${sig.name} ${sig.score.toFixed(0)}</span>`
).join('')}
</div>` : ''}
...
function _daysSince(iso: string): string {
    if (!iso) return '';
    const now = new Date();
