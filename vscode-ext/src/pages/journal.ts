...
function normalizeEntry(raw: any): any {
    ...
    return {
        ...
        score: finiteScore(source.ai_score),
...
function renderJournalEntry(entry: any): string {
    ...
    const score = finiteScore(entry.score);
...
<div class="flex-row gap-8"><span style="color:#6B7280;font-size:11px">${escapeHtml(entry.date)}</span><span class="${scoreTone(score)}" style="font-weight:700;font-size:18px;color:${scoreColor}">${scoreText(score, '分')}</span></div>
</div>
