...
export function buildBacktestPage(data: any): string {
    ...
    const winCount = numberValue(m.winning, trades.filter((trade: any) => numberValue(trade?.profit_pct) > 0).length);
...
<div class="grid2" style="padding:0;margin-bottom:8px">
  ...
  <div class="card">
    ...
    ${limitations.length ? `<div class="alert alert-warn" style="margin-top:12px">${limitations.map((item: unknown) => `• ${escapeHtml(item)}`).join('<br>')}</div>` : ''}
  </div>
</div>
...
<td>${numberValue(trade.entry_signal_score).toFixed(1)}</td><td>${numberValue(trade.holding_days)}天</td><td>${escapeHtml(trade.exit_reason || '-')}</td>
    ...
    return pageShell('backtest', 'Backtest · 可核验策略回测', content);
}
