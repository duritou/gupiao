...
export function buildPortfolioPage(data: any): string {
    ...
    const rejectionReason = (value: string): string => ({
    ...
    } as Record<string, string>)[value] || value || '未说明';
...
<div class="card"><h3>AI评分</h3><div class="metric-value ${scoreTone(finiteScore(pf.avg_ai_score))}">${scoreText(pf.avg_ai_score)}</div><span class="text-sm text-muted">${pf.position_count || 0}只持仓</span></div>
</div>
...
${positions.map((p: any, i: number) => {
    ...
    const positionScore = finiteScore(p.ai_score);
    const scClass = scoreTone(positionScore);
...
<td><span class="${scClass}" style="font-weight:700;font-size:15px">${scoreText(positionScore)}</span> <span style="font-size:11px;color:${changeColor}">${changeArrow}</span></td>
