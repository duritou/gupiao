...
let pipelineSyncTimer: NodeJS.Timeout | null = null;
let pipelineSyncInFlight: Promise<void> | null = null;
...
type StockSnapshot = {
    watchScores: any | null;
    liveQuotes: any | null;
};
type StockSnapshotCacheEntry = {
    data: StockSnapshot | null;
    fetchedAt: number;
    pending: Promise<StockSnapshot> | null;
};
const stockSnapshotCache = new Map<string, StockSnapshotCacheEntry>();
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push({ dispose: () => {
        if (pipelineSyncTimer) clearInterval(pipelineSyncTimer);
...
function probeBackend(): Promise<boolean> {
    ...
    healthProbeInFlight = healthCheck()
        .catch(() => false)
...
async function pollPipelineVersion(): Promise<void> {
    if (pipelineSyncInFlight) return pipelineSyncInFlight;
    pipelineSyncInFlight = (async () => {
        const snapshot = await httpGet('/ai-os/latest-run').catch(() => null);
...
async function requestManagedRestart(root: string): Promise<void> {
...
async function getTodayAlerts(force = false): Promise<any | null> {
    const now = Date.now();
    if (!force && todayAlertsCache.data && now - todayAlertsCache.fetchedAt < TODAY_ALERTS_CACHE_TTL_MS) {
        return todayAlertsCache.data;
    }
    if (todayAlertsCache.pending) return todayAlertsCache.pending;

    const pending = httpGet('/alerts/today').catch(() => null);
...
async function getStockSnapshot(codes: string[], force = false): Promise<StockSnapshot> {
    const normalizedCodes = Array.from(new Set(codes.map(normalizeCode).filter(
        (code): code is string => Boolean(code),
    ))).sort();
    if (!normalizedCodes.length) return { watchScores: null, liveQuotes: null };
    ...
    const pending = Promise.all([
        httpPost('/signals/batch', { codes: normalizedCodes, force }, 10_000).catch(() => null),
        httpPost('/market/quotes', { codes: normalizedCodes }, 10_000).catch(() => null),
        ...
        switch (page) {
            case 'dashboard': {
                ...
                const [market, scanner, stockSnapshot, brief, alerts, trackRecord, aiAlpha, userProfile, dataQuality] = await Promise.all([
                    httpGet('/market/overview', 10_000).catch(() => null),
            ...
            case 'review_lab': {
                const wechat = await httpGet('/knowledge/wechat/review?limit=200').catch(() => null);
                return { wechat };
            }
            case 'health': {
...
function handleMessage(msg: any, currentPage: string) {
    switch (msg.command) {
        ...
        case 'wechatRefresh': void refreshWechatKnowledge(); break;
    }
}

async function refreshWechatKnowledge() {
    try {
        const result = await httpPost('/knowledge/wechat/refresh', undefined, 210_000);
