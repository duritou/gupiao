...
- 优化前备份提交：`75759e1`
- 当前方案提交：`95c9194`（财务补缺待提交）
- 当前源码 HEAD：`95c9194`（日线 fallback 已提交，财务补缺在工作区）
...
Phase 0–3 已完成，Phase 4 已完成估值/龙虎榜路由和 RemoteMarketDiscovery 特色数据的可切换入口；Phase 5 已完成日线 fallback 和财务“只补空缺、不覆盖”最小接入。当前默认 shadow/validator：HiThink 观察写入 `provider_observations`，日线/财务只在满足开关和失败条件时进入真实 fallback，不改变候选、排序、分数、Top N 或交易门槛；下一步补齐财务指标校验、跨源一致性摘要和交易日 canary。
