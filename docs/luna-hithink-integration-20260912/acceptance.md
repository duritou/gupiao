...
- 优化前备份提交：`75759e1`
- 当前方案提交：`6c5a296`
- 当前源码 HEAD：`6c5a296`（日线/财务 fallback、财务指标一致性审计、canary 探针、消歧与健康接口及基础设施复跑已提交）
...
| Phase 6 交易日 canary | `in_progress` | 已提供 `scripts/probe_hithink.py`、脱敏健康接口和离线契约测试；至少 3 个完整交易日、300 个受控样本尚未收集 |
...
Phase 0–3 已完成，Phase 4 已完成估值/龙虎榜路由、RemoteMarketDiscovery 特色数据入口和市场代码/名称消歧入口；Phase 5 已完成日线 fallback、财务“只补空缺、不覆盖”和财务指标一致性审计最小接入；Phase 6 已提供安全有界探针和脱敏健康接口，进入交易日样本收集阶段。当前默认 shadow/validator：HiThink 观察写入 `provider_observations`，日线/财务只在满足开关和失败条件时进入真实 fallback，不改变候选、排序、分数、Top N 或交易门槛；下一步补齐全量跨源一致性摘要、更多特色 API 和 3 个完整交易日 canary。
