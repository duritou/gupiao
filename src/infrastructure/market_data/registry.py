...
DATA_SOURCE_REGISTRY: dict[str, DataSource] = {
    ...
    "tickflow": DataSource(
        id="tickflow", name="TickFlow", name_en="TickFlow",
        url="https://tickflow.org", layer="market", tier=TrustTier.COMMERCIAL,
        access=AccessMethod.API, category="quote",
        provides=["实时行情", "日K", "分钟K", "标的池", "部分财务指标"],
        update_frequency="realtime", rate_limit="按套餐配额",
        requires_auth=True, is_free=False, base_trust=0.90,
        integration_status="active",
        notes="标准 API 行情源；需配置 TICKFLOW_API_KEY，失败时回退腾讯/新浪。",
    ),
    "tushare": DataSource(
        id="tushare", name="Tushare Pro", name_en="Tushare",
        url="https://tushare.pro", layer="market", tier=TrustTier.COMMUNITY,
        access=AccessMethod.SDK, category="quote",
        provides=["日K", "每日指标", "财务数据", "个股资金流", "指数", "涨跌停"],
        update_frequency="daily", rate_limit="200次/分钟（按接口权限）",
