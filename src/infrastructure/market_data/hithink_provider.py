...
class HiThinkProvider(HiThinkClient):
    ...
    async def fetch_symbol_search(self, query: str, limit: int = 10) -> HiThinkPayload:
        ...
        data, request_id, _ = await self._send(
            "symbol_search",
            "/api/meta/tickers/search",
            {"q": str(query).strip(), "limit": int(limit)},
        )
        items = data.get("item") if isinstance(data, dict) else None
        if not isinstance(items, list):
            raise HiThinkError(
                "symbol_item_not_array", category="contract_error", request_id=request_id
            )
        stamp = data.get("timestamp") if isinstance(data, dict) else None
        return HiThinkPayload(
            [map_symbol(row) for row in items if isinstance(row, dict)],
            "symbol_search",
            "/api/meta/tickers/search",
            request_id,
            stamp,
            timestamp_iso(stamp)[:10],
            False,
            len(items),
            iso_now(),
        )
    ...
    async def fetch_snapshot(self, codes: str | list[str] | tuple[str, ...]) -> HiThinkPayload:
        ...
        data, request_id, _ = await self._send(
            "snapshot", "/api/a-share/prices/snapshot", {"thscodes": ",".join(normalized)}
        )
        if not isinstance(data, dict) or not isinstance(data.get("item"), list):
            raise HiThinkError(
                "snapshot_item_not_array", category="contract_error", request_id=request_id
            )
        stamp = data.get("timestamp")
        return HiThinkPayload(
            [map_snapshot(row, stamp) for row in data["item"] if isinstance(row, dict)],
            "snapshot",
    ...
    async def fetch_daily_history(
        self, code: str, start: Any, end: Any, adjust: str = "forward"
    ) -> HiThinkPayload:
        ...
        if not isinstance(data, dict) or not isinstance(data.get("item"), list):
            raise HiThinkError(
                "history_item_not_array", category="contract_error", request_id=request_id
            )
    ...
    async def fetch_financial_history(
        self, code: str, period: str = "annual", limit: int = 4
    ) -> HiThinkPayload:
        thscode = normalize_thscode(code)
        ...
        for name, path in FINANCIAL_PATHS.items():
            data, request_id, _ = await self._send(
                ...
                path,
                {"thscode": thscode, "period": period, "limit": int(limit)},
            )
            if not isinstance(data, dict) or not isinstance(data.get("item"), list):
                raise HiThinkError(
                    f"{name}_item_not_array", category="contract_error", request_id=request_id
                )
            statements[name] = [
                map_financial_row(row) for row in data["item"] if isinstance(row, dict)
            ]
        ...
        return HiThinkPayload(
            {"statements": statements, "period": period, "limit": int(limit)},
            "financial_statements",
            ";".join(FINANCIAL_PATHS),
            ";".join(request_ids)[:120],
            latest,
            timestamp_iso(latest)[:10],
            False,
            sum(len(rows) for rows in statements.values()),
            iso_now(),
        )
    ...
    async def fetch_financial_indicators(self, code: str, report: str) -> HiThinkPayload:
        ...
        if not isinstance(data, dict) or not isinstance(data.get("abilities"), list):
            raise HiThinkError(
                "abilities_not_array", category="contract_error", request_id=request_id
            )
        abilities = []
        for block in data["abilities"]:
            if not isinstance(block, dict) or not isinstance(block.get("indicators"), list):
                raise HiThinkError(
                    "indicators_not_array", category="contract_error", request_id=request_id
                )
            abilities.append(
                {
                    "ability": block.get("ability"),
                    "indicators": [
                        {"index_id": item.get("index_id"), "value": item.get("value")}
    ...
    async def fetch_valuation_snapshot(
        self, codes: str | list[str] | tuple[str, ...]
    ) -> HiThinkPayload:
        normalized = normalize_codes(codes)
        data, request_id, _ = await self._send(
            "valuation", "/api/a-share/valuations/snapshot", {"thscodes": ",".join(normalized)}
        )
        if not isinstance(data, dict) or not isinstance(data.get("item"), list):
            raise HiThinkError(
                "valuation_item_not_array", category="contract_error", request_id=request_id
            )
        stamp = data.get("timestamp")
        return HiThinkPayload(
            [map_valuation(row) for row in data["item"] if isinstance(row, dict)],
            "valuation",
    ...
    async def fetch_limit_pool(
        self, kind: str = "up", trade_date: Any = None, page: int = 1, size: int = 50
    ) -> HiThinkPayload:
        ...
        if not isinstance(data, dict) or not isinstance(data.get("item"), list):
            raise HiThinkError(
                "limit_pool_item_not_array", category="contract_error", request_id=request_id
            )
        stamp = data.get("timestamp")
        rows = [map_pool_row(row) for row in data["item"] if isinstance(row, dict)]
        return HiThinkPayload(
            {"pagination": data.get("pagination") or {}, "item": rows},
            "special_data",
            path,
            request_id,
            stamp,
    ...
    async def fetch_dragon_tiger(
        self, trade_date: Any = None, board_type: str = "all"
    ) -> HiThinkPayload:
        ...
        data, request_id, _ = await self._send(
            "dragon_tiger", "/api/a-share/special-data/dragon-tiger-list", params
        )
        if not isinstance(data, dict):
            raise HiThinkError(
                "dragon_tiger_data_not_object", category="contract_error", request_id=request_id
            )
        ...
        if not isinstance(stock_items, list) or not isinstance(hot_money_items, list):
            raise HiThinkError(
                "dragon_tiger_items_not_array", category="contract_error", request_id=request_id
            )
        stamp = data.get("timestamp")
        return HiThinkPayload(
            {
                "board_type": data.get("board_type", board_type),
                "trade_date": data.get("trade_date"),
                "count": data.get("count"),
                "stock_count": data.get("stock_count"),
                "stock_items": [map_pool_row(row) for row in stock_items if isinstance(row, dict)],
                "hot_money_items": hot_money_items,
            },
            "dragon_tiger",
            "/api/a-share/special-data/dragon-tiger-list",
            request_id,
            stamp,
            date_text(data.get("trade_date")),
            False,
            len(stock_items),
            iso_now(),
        )
