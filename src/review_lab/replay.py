...
def replay(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for code in sorted({row["ts_code"] for row in rows}):
        bars = sorted(
            [row for row in rows if row["ts_code"] == code], key=lambda row: row["trade_date"]
        )
        if len(bars) != 3:
        ...
        if any(
            not math.isfinite(float(b[k] or 0)) or float(b[k] or 0) <= 0
            for b in bars
        ...
        if any(abs(b["open"] / p["close"] - 1) >= 0.09 for p, b in ((signal, buy), (buy, sell))):
        ...
        proceeds = round(
            exit_price * quantity
            - max(5, exit_price * quantity * 0.0003)
            - exit_price * quantity * 0.0005,
            2,
        )
        ...
        display_name = str(signal.get("name") or buy.get("name") or sell.get("name") or code)
        trades.append(
            {
                "symbol": code,
                "name": display_name,
    ...
    return {
        ...
        "trades": trades,
        "rejected": rejected,
        "net_pnl": round(cash - 100000, 2),
        "execution": "assumed_daily_open_not_verified_fills",
        "learning": [
            f"本次合格模拟往返 {len(trades)} 笔；禁止由小样本推断策略有效。",
